...
export default function OnboardingPage() {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4" style={{ background: 'linear-gradient(135deg, #0a0a0f 0%, #12121a 50%, #0a0a0f 100%)' }}>
        <div className="text-center">
          <div className="inline-block animate-spin rounded-full h-12 w-12 sm:h-16 sm:w-16 border-b-4 border-emerald-500 mb-4"></div>
  ...
  if (showRefinement && financialPlan) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-6 sm:py-8" style={{ background: 'linear-gradient(135deg, #0a0a0f 0%, #12121a 50%, #0a0a0f 100%)' }}>
        <div className="max-w-2xl w-full glass-card p-5 sm:p-6 md:p-8" style={{ background: 'rgba(18, 18, 26, 0.95)', border: '1px solid rgba(139, 92, 246, 0.2)' }}>
          <div className="mb-5 sm:mb-6">
            <button
              onClick={() => setShowRefinement(false)}
              className="text-emerald-400 hover:text-emerald-300 flex items-center gap-2 mb-4 text-sm sm:text-base"
            >
          ...
          <form onSubmit={handleRefinementSubmit} className="space-y-5 sm:space-y-6">
            {accumulatedContext && (
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-2 flex items-center gap-1">
                <LightbulbIcon sx={{ fontSize: 16, color: '#fbbf24' }} /> Be specific about your situation - the more details you provide, the better we can tailor your plan
              </p>
            </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
              <button
                type="button"
                onClick={() => setShowRefinement(false)}
                className="px-6 py-3 rounded-xl font-medium text-gray-300 transition-all order-2 sm:order-1"
              ...
              <button
                type="submit"
                className="flex-1 px-6 py-3.5 sm:py-3 rounded-xl font-semibold text-white order-1 sm:order-2"
  ...
  // Edit info form when user wants to change goal, income, or currency
  if (showEditInfo && financialPlan) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-6 sm:py-8" style={{ background: 'linear-gradient(135deg, #0a0a0f 0%, #12121a 50%, #0a0a0f 100%)' }}>
        <div className="max-w-md w-full glass-card p-5 sm:p-6 md:p-8" style={{ background: 'rgba(18, 18, 26, 0.95)', border: '1px solid rgba(139, 92, 246, 0.2)' }}>
          <div className="mb-5 sm:mb-6">
            <button
              onClick={() => setShowEditInfo(false)}
              className="text-emerald-400 hover:text-emerald-300 flex items-center gap-2 mb-4 text-sm sm:text-base"
            >
              <ArrowBackIcon sx={{ fontSize: 18 }} /> Back to plan
            </button>
            <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2">
              Edit Your Information
            </h1>
            <p className="text-gray-400 text-sm sm:text-base">
              Update your goal, income, or currency to regenerate your plan
            </p>
          </div>

          {error && (
            <div className="mb-5 sm:mb-6 p-3 sm:p-4 rounded-xl text-rose-300 text-sm sm:text-base" style={{ background: 'rgba(239, 68, 68, 0.1)', border: '1px solid rgba(239, 68, 68, 0.3)' }}>
              {error}
            </div>
          )}
          ...
          <form onSubmit={async (e) => {
            e.preventDefault();
            // Reset accumulated context since we're starting fresh with new info
            setAccumulatedContext("");
            setOriginalGoal(editFormData.goal);
            setOriginalIncome(editFormData.monthlyIncome);
            setOriginalCurrency(editFormData.currency);
            ...
            try {
              const response = await fetch("/api/financial-goals", {
                ...
                body: JSON.stringify({
                  ...
                  monthlyIncome: parseFloat(editFormData.monthlyIncome),
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setEditFormData({ ...editFormData, monthlyIncome: e.target.value })
                }
                placeholder="5000"
                className="modern-input"
              />
  ...
  if (financialPlan) {
    ...
    return (
      <div className="min-h-screen py-6 sm:py-8 md:py-12 px-4" style={{ background: 'linear-gradient(135deg, #0a0a0f 0%, #12121a 50%, #0a0a0f 100%)' }}>
        <div className="max-w-5xl mx-auto">
          <div className="glass-card p-4 sm:p-6 md:p-8 animate-fade-in-up" style={{ background: 'rgba(18, 18, 26, 0.8)', border: '1px solid rgba(16, 185, 129, 0.2)' }}>
            <h1 className="text-2xl sm:text-3xl font-bold text-white mb-2 animate-slide-in">
              Your Personalized Financial Plan
            </h1>
            <p className="text-gray-400 mb-6 sm:mb-8 text-sm sm:text-base animate-slide-in" style={{animationDelay: '0.1s'}}>
              Here's what we recommend based on your goals
            </p>

            {/* Goal Section */}
            <div className="mb-6 sm:mb-8 p-4 sm:p-6 rounded-xl animate-fade-in" style={{animationDelay: '0.2s', background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.1) 0%, rgba(245, 158, 11, 0.1) 100%)', border: '1px solid rgba(16, 185, 129, 0.2)'}}>
              <div className="flex items-start justify-between">
                <div>
                  <h2 className="text-lg sm:text-xl font-semibold text-emerald-300 mb-2">
                    Your Goal
                  </h2>
                  <p className="text-base sm:text-lg text-white">{financialPlan.Goal}</p>
                ...
                <button
                  onClick={() => {
                    setEditFormData({
                      goal: originalGoal || financialPlan.Goal,
                      monthlyIncome: originalIncome || financialPlan.MonthlyIncome.toString(),
                      currency: originalCurrency || financialPlan.Currency,
                    });
                ...
                >
                  <EditIcon sx={{ fontSize: 20, color: '#10b981' }} />
                </button>
              </div>
            </div>
            ...
            {/* Visual Chart */}
            <div className="mb-6 sm:mb-8 p-4 sm:p-6 rounded-xl animate-fade-in" style={{animationDelay: '0.4s', background: 'rgba(255, 255, 255, 0.02)'}}>
              ...
              <div className="flex flex-col items-center gap-6 md:flex-row md:gap-8">
                ...
                <div className="relative w-48 h-48 sm:w-64 sm:h-64 flex-shrink-0 animate-scale-in" style={{animationDelay: '0.5s'}}>
                  ...
                  <div className="absolute inset-0 flex items-center justify-center flex-col">
                    <div className="text-xl sm:text-3xl font-bold text-white">{financialPlan.Currency}</div>
                    <div className="text-lg sm:text-2xl font-bold gradient-text">{totalIncome.toLocaleString()}</div>
                    <div className="text-[10px] sm:text-xs text-gray-500 mt-1">Monthly Income</div>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-3 w-full">
                  <div className="flex items-center gap-3 animate-slide-in-right" style={{animationDelay: '0.6s'}}>
                    <div className="w-5 h-5 sm:w-6 sm:h-6 bg-emerald-500 rounded flex-shrink-0"></div>
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-white text-sm sm:text-base">Essential Expenses</div>
                  ...
                  <div className="flex items-center gap-3 animate-slide-in-right" style={{animationDelay: '0.7s'}}>
                    <div className="w-5 h-5 sm:w-6 sm:h-6 bg-amber-500 rounded flex-shrink-0"></div>
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-white text-sm sm:text-base">Savings</div>
                      <div className="text-xs sm:text-sm text-gray-400">
                  ...
                  <div className="flex items-center gap-3 animate-slide-in-right" style={{animationDelay: '0.8s'}}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-white text-sm sm:text-base">Discretionary Spending</div>
                      <div className="text-xs sm:text-sm text-gray-400">
            ...
            <div className="mb-6 sm:mb-8 animate-fade-in" style={{animationDelay: '0.9s'}}>
              <h2 className="text-lg sm:text-xl font-semibold text-white mb-4">
              ...
              <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
                {/* Essential Expenses */}
                <div className="p-4 sm:p-5 rounded-xl transition-transform duration-300 hover:scale-105 hover:shadow-lg animate-fade-in" style={{animationDelay: '1s', background: 'rgba(16, 185, 129, 0.1)', border: '1px solid rgba(16, 185, 129, 0.3)'}}>
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-emerald-300 text-sm sm:text-base">
                      Essential Expenses
                    </h3>
                    <HomeIcon sx={{ fontSize: { xs: 24, sm: 28 }, color: '#10b981' }} />
                  </div>
                ...
                {/* Savings */}
                <div className="p-4 sm:p-5 rounded-xl transition-transform duration-300 hover:scale-105 hover:shadow-lg animate-fade-in" style={{animationDelay: '1.1s', background: 'rgba(245, 158, 11, 0.1)', border: '1px solid rgba(245, 158, 11, 0.3)'}}>
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-amber-300 text-sm sm:text-base">Savings</h3>
                    <SavingsIcon sx={{ fontSize: { xs: 24, sm: 28 }, color: '#f59e0b' }} />
                  </div>
                  <p className="text-2xl sm:text-3xl font-bold text-white mb-2">
                ...
                <div className="p-4 sm:p-5 rounded-xl transition-transform duration-300 hover:scale-105 hover:shadow-lg animate-fade-in sm:col-span-2 md:col-span-1" style={{animationDelay: '1.2s', background: 'rgba(132, 204, 22, 0.1)', border: '1px solid rgba(132, 204, 22, 0.3)'}}>
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-semibold text-lime-300 text-sm sm:text-base">
                      Discretionary
                    </h3>
                    <CelebrationIcon sx={{ fontSize: { xs: 24, sm: 28 }, color: '#84cc16' }} />
                  </div>
                  <p className="text-2xl sm:text-3xl font-bold text-white mb-2">
            ...
            <div className="mb-6 p-4 sm:p-6 rounded-xl animate-fade-in" style={{animationDelay: '1.3s', background: 'linear-gradient(135deg, rgba(16, 185, 129, 0.15) 0%, rgba(245, 158, 11, 0.15) 100%)', border: '1px solid rgba(16, 185, 129, 0.3)'}}>
              ...
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center">
                <button
                  ...
                  className="px-6 sm:px-8 py-3.5 sm:py-3 rounded-xl font-semibold flex items-center justify-center gap-2 hover:scale-105 transform transition text-white text-sm sm:text-base"
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-6 sm:py-8" style={{ background: 'linear-gradient(135deg, #0a0a0f 0%, #12121a 50%, #0a0a0f 100%)' }}>
      <div className="max-w-md w-full glass-card p-5 sm:p-6 md:p-8" style={{ background: 'rgba(18, 18, 26, 0.95)', border: '1px solid rgba(139, 92, 246, 0.2)' }}>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
          ...
          <button
            type="submit"
            className="w-full py-3.5 sm:py-3 px-6 rounded-xl font-semibold text-base sm:text-lg text-white transition-all hover:scale-[1.02] active:scale-[0.98]"
            style={{ background: 'linear-gradient(135deg, #10b981 0%, #f59e0b 100%)' }}
          >
